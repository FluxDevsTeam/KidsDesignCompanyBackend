...
class ApiFactoryManagerDashboard(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        expenses_month = sum([sold_cost_month + salary_costs_month, contractor_costs_month, other_expenses_month + other_production_expensis_month])
        ...
        total_expenses_year = sum([sold_cost_year + salary_costs_year, contractor_costs_year, other_expenses_year + other_production_expensis_year])
...
class CEODashboardViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        total_expenses_month = sum([sold_cost_month + salary_costs_month, contractor_costs_month, other_expenses_month + other_production_expensis_month])
        ...
        data = {
            'key_metrics': {
                ...
                'current_assets_value': round(asset_analysis.get('active', 0), 2) if asset_analysis.get('active') is not None else 0,
            ...
            'income_breakdown_year': {
                ...
                'no_shop_projects': round(no_shop_projects_income_year, 2) if no_shop_projects_income_year is not None else 0,
                'shop_sales': round(total_shop_income_year, 2) if total_shop_income_year is not None else 0,
                'non_project_shop_sales': round(total_non_project_shop_income_year, 2) if total_non_project_shop_income_year is not None else 0,
                'percentage_projects': round((no_shop_projects_income_year / total_income_year * 100) if total_income_year else 0, 2) if no_shop_projects_income_year is not None and total_income_year is not None else 0,
                'percentage_shop': round((total_shop_income_year / total_income_year * 100) if total_income_year else 0, 2) if total_shop_income_year is not None and total_income_year is not None else 0
            },
            'income_breakdown_month': {
                'projects': round(total_projects_income_month, 2) if total_projects_income_month is not None else 0,
                'no_shop_projects': round(no_shop_projects_income_month, 2) if no_shop_projects_income_month is not None else 0,
                'shop_sales': round(total_shop_income_month, 2) if total_shop_income_month is not None else 0,
                'non_project_shop_sales': round(total_non_project_shop_income_month, 2) if total_non_project_shop_income_month is not None else 0,
                'percentage_projects': round((no_shop_projects_income_month / total_income_month * 100) if total_income_month else 0, 2) if no_shop_projects_income_month is not None and total_income_month is not None else 0,
                'percentage_shop': round((total_shop_income_month / total_income_month * 100) if total_income_month else 0, 2) if total_shop_income_month is not None and total_income_month is not None else 0
            },
            'expense_breakdown_year': {
                ...
                'other_production_expensis': round(other_production_expensis_year, 2) if other_production_expensis_year is not None else 0,
...
class ProjectManagerDashboardViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        total_project_expenses_year = sum([overhead_cost_year + sold_cost_year, contractor_costs_year, expenses_year + other_production_expensis_year])
        ...
        total_project_expenses_month = sum([overhead_cost_month + sold_cost_month, contractor_costs_month, expenses_month + other_production_expensis_month])
